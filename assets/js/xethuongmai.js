        ...
        productsToRender.forEach(xe => {
            ...
            const productHtmls = `
                <div class="grid__column-9-3">
                    <div class="product">
                        <a href="car-product.html?type=xethuongmai&id=${xe.id}" class="product-img" style="background-image: url(${xe.background_img});"></a>
...
async function fetchProducts() {
    ...
    const xethuongmaiRef = ref(database, 'product/xeoto/xethuongmai');
    
    onValue(xethuongmaiRef, (snap) => {
        const xethuongmaiData = snap.val();
        if (xethuongmaiData) {
            allProducts = Object.keys(xethuongmaiData).map(key => ({...xethuongmaiData[key], id: key}));
