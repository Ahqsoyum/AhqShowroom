        ...
        productsToRender.forEach(xe => {
            ...
            const productHtmls = `
                <div class="grid__column-9-3">
                    <div class="product">
                        <a href="car-product.html?type=xedulich&id=${xe.id}" class="product-img" style="background-image: url(${xe.background_img});"></a>
...
async function fetchProducts() {
    ...
    const xedulichRef = ref(database, 'product/xeoto/xedulich');
    
    onValue(xedulichRef, (snap) => {
        const xedulichData = snap.val();
        if (xedulichData) {
            allProducts = Object.keys(xedulichData).map(key => ({...xedulichData[key], id: key}));
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Sửa lỗi: Di chuyển code sắp xếp vào đây và sử dụng querySelectorAll
